...
__export(index_exports, {
  ...
  ArkadeLightningMessageHandler: () => ArkadeLightningMessageHandler,
  ...
  ServiceWorkerArkadeLightning: () => ServiceWorkerArkadeLightning,
  SwapError: () => SwapError,
...
function claimVHTLCIdentity(identity, preimage) {
  return {
    ...
    sign: async (tx, inputIndexes) => {
      ...
      signedTx = import_sdk3.Transaction.fromPSBT(signedTx.toPSBT());
      if (preimage) {
        for (const inputIndex of inputIndexes || Array.from(
          { length: signedTx.inputsLength },
          (_, i) => i
        )) {
          (0, import_sdk3.setArkPsbtField)(signedTx, inputIndex, import_sdk3.ConditionWitness, [
            preimage
          ]);
        }
      }
...
function createVHTLCBatchHandler(intentId, vhtlc, arkProvider, identity, session, sweepPublicKey, forfeitOutputScript, connectorIndex = 0) {
  ...
  return {
    onBatchStarted: async (event) => {
      ...
      const sweepTapscript = import_sdk4.CSVMultisigTapscript.encode({
        timelock: {
      ...
      sweepTapTreeRoot = (0, import_payment.tapLeafHash)(sweepTapscript);
      return { skip: false };
    },
    ...
    onBatchFinalization: async (event, _, connectorTree) => {
      ...
      (0, import_sdk4.validateConnectorsTxGraph)(event.commitmentTx, connectorTree);
...
function createForfeitTx(input, forfeitOutputScript, connector) {
  ...
  const sequence = (0, import_sdk4.getSequence)(input.tapLeafScript);
  return (0, import_sdk4.buildForfeitTx)(
    [
      {
        txid: input.txid,
        index: input.vout,
...
var IndexedDbSwapRepository = class {
  ...
  async getDB() {
    ...
    this.db = await (0, import_sdk5.openDatabase)(this.dbName, DB_VERSION, initDatabase);
    return this.db;
  }
  async saveSwap(swap) {
  ...
  async [Symbol.asyncDispose]() {
    if (!this.db) return;
    await (0, import_sdk5.closeDatabase)(this.dbName);
    this.db = null;
  }
};
...
var ArkadeLightning = class {
  ...
  async claimVHTLC(pendingSwap) {
    ...
    const output = {
      ...
      script: import_sdk6.ArkAddress.decode(address).pkScript
    };
    ...
    var finalStatus;
    if ((0, import_sdk6.isRecoverable)(vtxo)) {
  ...
  async refundVHTLC(pendingSwap) {
    ...
    const vhtlcPkScript = import_sdk6.ArkAddress.decode(
    ...
    if (!vhtlcScript)
      throw new Error("Failed to create VHTLC script for reverse swap");
    const isRecoverableVtxo = (0, import_sdk6.isRecoverable)(vtxo);
    const input = {
      ...vtxo,
  ...
  async joinBatch(identity, input, output, {
    ...
    const intentInput = {
      ...
      tapLeafScript: [input.tapLeafScript],
      unknown: [import_sdk6.VtxoTaprootTree.encode(input.tapTree)],
      sequence: (0, import_sdk6.getSequence)(input.tapLeafScript)
    };
    const registerIntent = import_sdk6.Intent.create(
      intentMessage,
      [intentInput],
      [output]
    );
    const deleteIntent = import_sdk6.Intent.create(deleteMessage, [intentInput]);
    ...
    const decodedAddress = (0, import_btc_signer3.Address)(
      network in import_sdk6.networks ? import_sdk6.networks[network] : import_sdk6.networks.bitcoin
    ...
    try {
      ...
      const commitmentTxid = await import_sdk6.Batch.join(eventStream, handler, {
        abortController
      });
      logger.log(
  ...
  async claimVHTLCwithOffchainTx(vhtlcIdentity, vhtlcScript, serverXOnlyPublicKey, input, output, arkInfos) {
    ...
    const serverUnrollScript = import_sdk6.CSVMultisigTapscript.decode(
      rawCheckpointTapscript
    );
    const { arkTx, checkpoints } = (0, import_sdk6.buildOffchainTx)(
      [input],
      [output],
      serverUnrollScript
    );
  ...
  async refundVHTLCwithOffchainTx(pendingSwap, boltzXOnlyPublicKey, ourXOnlyPublicKey, serverXOnlyPublicKey, input, output, arkInfos) {
    ...
    const { arkTx: unsignedRefundTx, checkpoints: checkpointPtxs } = (0, import_sdk6.buildOffchainTx)([input], [output], serverUnrollScript);
    ...
    const combinedSignedRefundTx = (0, import_sdk6.combineTapscriptSigs)(
      boltzSignedRefundTx,
      signedRefundTx
    );
    const combinedSignedCheckpointTx = (0, import_sdk6.combineTapscriptSigs)(
    ...
    const finalCheckpointTx = (0, import_sdk6.combineTapscriptSigs)(
  ...
  createVHTLCScript({
    ...
    const vhtlcScript = new import_sdk6.VHTLC.Script({
...
var ArkadeLightningMessageHandler = class _ArkadeLightningMessageHandler {
  ...
  async handleInit({ payload }) {
    ...
    this.arkProvider = new import_sdk7.RestArkProvider(arkServerUrl);
    this.indexerProvider = new import_sdk7.RestIndexerProvider(arkServerUrl);
